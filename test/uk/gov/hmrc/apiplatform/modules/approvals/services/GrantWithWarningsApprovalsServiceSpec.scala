...
class GrantWithWarningsApprovalsServiceSpec extends AsyncHmrcSpec {
  trait Setup extends AuditServiceMockModule 
    ...
    val underTest = new GrantWithWarningsApprovalsService(AuditServiceMock.aMock, ApplicationRepoMock.aMock, StateHistoryRepoMock.aMock, SubmissionsServiceMock.aMock, EmailConnectorMock.aMock)
  }
  ...
  "GrantApprovalsService" should {
    "grant the specified application" in new Setup {
      ...
      val result = await(underTest.grantWithWarnings(applicationPendingGKApproval, submittedSubmission, gatekeeperUserName, warnings))

      result should matchPattern {
        case GrantWithWarningsApprovalsService.Actioned(app) if(app.state.name == PENDING_REQUESTER_VERIFICATION) =>
      }
    ...
    "fail to grant the specified application if the application is in the incorrect state" in new Setup {
      val result = await(underTest.grantWithWarnings(anApplicationData(applicationId, testingState()), answeredSubmission, gatekeeperUserName, warnings))

      result shouldBe GrantWithWarningsApprovalsService.RejectedDueToIncorrectApplicationState
    }

    "fail to grant the specified application if the submission is not in the submitted state" in new Setup {
      val result = await(underTest.grantWithWarnings(applicationPendingGKApproval, answeredSubmission, gatekeeperUserName, warnings))

      result shouldBe GrantWithWarningsApprovalsService.RejectedDueToIncorrectSubmissionState
    }
  }
}