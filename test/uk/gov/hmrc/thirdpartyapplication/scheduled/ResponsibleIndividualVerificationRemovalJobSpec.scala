...
class ResponsibleIndividualVerificationRemovalJobSpec extends AsyncHmrcSpec with BeforeAndAfterAll with ApplicationStateUtil
  ...
  "ResponsibleIndividualVerificationRemovalJob" should {
    "send email and remove database record" in new Setup {
      ...
      val verification =
        ResponsibleIndividualToUVerification(ResponsibleIndividualVerificationId(code), app.id, completelyAnswerExtendedSubmission.submission.id, 0, app.name, FixedClock.now)
      ResponsibleIndividualVerificationRepositoryMock.FetchByTypeStateAndAge.thenReturn(verification)

      await(job.runJob)

      ResponsibleIndividualVerificationRepositoryMock.FetchByTypeStateAndAge.verifyCalledWith(ResponsibleIndividualVerification.VerificationTypeToU, REMINDERS_SENT, FixedClock.now.minus(removalInterval.toSeconds, SECONDS))
    ...
    "continue after invalid records" in new Setup {
      ...
      val verification1 =
        ResponsibleIndividualToUVerification(ResponsibleIndividualVerificationId(code1), badApp.id, completelyAnswerExtendedSubmission.submission.id, 0, badApp.name, FixedClock.now)
      val code2 = "725446087565645698"
      val verification2 =
        ResponsibleIndividualToUVerification(ResponsibleIndividualVerificationId(code2), app.id, completelyAnswerExtendedSubmission.submission.id, 0, app.name, FixedClock.now)
