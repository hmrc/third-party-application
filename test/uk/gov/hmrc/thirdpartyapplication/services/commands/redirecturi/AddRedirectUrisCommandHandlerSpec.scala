  ...
  "AddRedirectUrisCommandHandler" when {
    "given a principal application" should {
      "succeed when application is standardAccess" in new Setup {
        ...
        val appWithFourUris = anApplicationData(applicationId).copy(access = Access.Standard(fourUris), collaborators = devAndAdminCollaborators)
      ...
      "fail when we try to add a sixth URI" in new Setup {
        ...
        val appWithFiveUris = anApplicationData(applicationId).copy(access = Access.Standard(fiveUris), collaborators = devAndAdminCollaborators)
