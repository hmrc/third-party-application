...
class ChangeSandboxApplicationPrivacyPolicyUrlCommandHandlerSpec extends CommandHandlerBaseSpec {
  ...
  trait Setup extends ApplicationRepositoryMockModule {
    ...
    val underTest = new ChangeSandboxApplicationPrivacyPolicyUrlCommandHandler(ApplicationRepoMock.aMock)
    ...
    def checkSuccessResult(expectedActor: Actors.AppCollaborator)(fn: => CommandHandler.AppCmdResultT) = {
      ...
      inside(testMe) { case (app, events) =>
        ...
        inside(event) {
          case ApplicationEvents.SandboxApplicationPrivacyPolicyUrlChanged(_, appId, eventDateTime, actor, anOldPrivacyPolicyUrl, aPrivacyPolicy) =>
            appId shouldBe applicationId
            actor shouldBe expectedActor
            anOldPrivacyPolicyUrl shouldBe Some(oldValue)
            aPrivacyPolicy shouldBe newValue
  ...
  "process" should {
    "create correct events for a valid request with a standard app" in new Setup {
      ApplicationRepoMock.UpdateLegacyPrivacyPolicyUrl.succeedsFor(Some(newValue))
