...
import uk.gov.hmrc.thirdpartyapplication.services.commands.clientsecret.AddClientSecretCommandHandler
...
class AddClientSecretCommandHandlerSpec extends CommandHandlerBaseSpec {
  ...
  class Setup(limit: Int = 3) extends ApplicationRepositoryMockModule {
    ...
    val addClientSecretByDev   = AddClientSecret(developerActor, "name", id, "hashed", instant)
