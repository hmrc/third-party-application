...
import uk.gov.hmrc.thirdpartyapplication.mocks.{ApplicationCommandServiceMockModule, AuditServiceMockModule}
...
class SubscriptionServiceSpec extends AsyncHmrcSpec with ApplicationStateUtil {
  ...
  "updateApplicationForApiSubscription" should {
    ...
    "return successfully using the correct CollaboratorActor if the collaborator is a member of the application" in new Setup {
      ...
      ApplicationCommandServiceMock.Update.thenReturnSuccess(applicationId, application)
      ...
      ApplicationCommandServiceMock.Update.verifyCalledWith(applicationId).asInstanceOf[SubscribeToApi].actor shouldBe actor
    }
    ...
    "return successfully using a GatekeeperUserCollaborator if there are no developers in the header carrier" in new SetupWithoutHc {
      ...
      ApplicationCommandServiceMock.Update.thenReturnSuccess(applicationId, applicationData)
    ...
    "throw an exception if the application has not updated" in new Setup {
      ...
      ApplicationCommandServiceMock.Update.thenReturnError(applicationId, errorMessage)
