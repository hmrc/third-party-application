...
import uk.gov.hmrc.apiplatform.modules.events.applications.domain.models.{ApplicationEvent, EventId, ApplicationEvents}
  ...
  protected trait BaseApplicationCommandDispatcherMock {
    ...
    object Dispatch {
      ...
      def thenReturnCommandSuccess(applicationData: ApplicationData) = {
        val dummyEvents                     =
          NonEmptyList.one(ApplicationEvents.RedirectUrisUpdatedV2(EventId.random, ApplicationId.random, FixedClock.instant, Actors.AppCollaborator("someuser".toLaxEmail), List.empty, List("new URI")))
