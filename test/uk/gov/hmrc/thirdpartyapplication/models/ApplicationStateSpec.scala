...
class ApplicationStateSpec extends HmrcSpec with ApplicationStateUtil with BeforeAndAfterEach with FixedClock {
  ...
  "state transition from TESTING " should {
    ...
    "move application to PENDING_GATEKEEPER_APPROVAL state" in {
      val resultState = startingState.toPendingGatekeeperApproval(upliftRequestedBy, clock)
  ...
  "state transition from PENDING_GATEKEEPER_APPROVAL " should {
    val startingState = pendingGatekeeperApprovalState(upliftRequestedBy).copy(updatedOn = LocalDateTime.now(clockMinusHours(24L)))
    ...
    "fail when application state is changed to PENDING_GATEKEEPER_APPROVAL" in {
      intercept[InvalidStateTransition](startingState.toPendingGatekeeperApproval(upliftRequestedBy, clock))
    }
  }
  ...
  "state transition from PRE_PRODUCTION " should {
    val startingState = preProductionState(upliftRequestedBy).copy(updatedOn = LocalDateTime.now(clockMinusHours(24L)))
  ...
  "state transition from PRODUCTION " should {
    val startingState = productionState(upliftRequestedBy).copy(updatedOn = LocalDateTime.now(clockMinusHours(24L)))
    "move back application to TESTING state" in {
