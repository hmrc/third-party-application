...
class ApplicationRepositorySearchISpec
  ...
  "Search" should {
    ...
    "return one application when 2 apps exist, but only one has the required userId" in {
      ...
      val collaborator1 = "user5@example.com".admin(userId1)
      val collaborator2 = "user2@example.com".admin(userId2)

      val prodApplication1 = anApplicationDataForTest(applicationId1).withCollaborators(collaborator1)
      val prodApplication2 = anApplicationDataForTest(applicationId2, prodClientId = ClientId("bbb")).withCollaborators(collaborator2)

      await(applicationRepository.save(prodApplication1))
      await(applicationRepository.save(prodApplication2))

      val applicationSearch = new ApplicationSearch(
        filters = List(ApplicationUserSearch),
        userId = Some(userId1)
      )

      val result = await(applicationRepository.searchApplications("testing")(applicationSearch))

      result.applications.size mustBe 1
      result.applications.head.id mustBe prodApplication1.id
      result.applications.map(
        _.collaborators.map(collaborator => collaborator.userId mustBe userId1)
      )
    }

    "return one application when 3 apps exist, but only one has the required userId, api subscription and is not deleted" in {
      val applicationId1     = ApplicationId.random
      val applicationId2     = ApplicationId.random
      ...
      val collaborator1 = "user1@example.com".admin(userId1)
      val collaborator2 = "user2@example.com".admin(userId2)

      val prodApplication1 = anApplicationDataForTest(applicationId1).withCollaborators(collaborator1)
      val prodApplication2 = anApplicationDataForTest(applicationId2, prodClientId = ClientId("bbb")).withCollaborators(collaborator1)
      val prodApplication3 = anApplicationDataForTest(applicationId3, prodClientId = ClientId("ccc")).withCollaborators(collaborator2)
      val prodApplication4 = anApplicationDataForTest(applicationId4, prodClientId = ClientId("ddd")).withCollaborators(collaborator1).withState(appStateDeleted)

      await(applicationRepository.save(prodApplication1))
      await(applicationRepository.save(prodApplication2))
      await(applicationRepository.save(prodApplication3))
      await(applicationRepository.save(prodApplication4))
      await(
        subscriptionRepository.collection
          .insertOne(
            aSubscriptionData(
      ...
      await(
        subscriptionRepository.collection
          .insertOne(
            aSubscriptionData(otherAPIContext, "version-1", prodApplication2.id)
          )
          .toFuture()
      )

      val applicationSearch = new ApplicationSearch(
        filters = List(ApplicationUserSearch, SpecificAPISubscription),
        userId = Some(userId1),
        apiContext = Some(expectedAPIContext.asContext),
        apiVersion = None
      )
