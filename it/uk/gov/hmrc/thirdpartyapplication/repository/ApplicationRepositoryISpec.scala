...
class ApplicationRepositoryISpec
  ...
  "addClientSecret" should {
    "append client secrets to an existing application" in {
      ...
      val clientSecret       =
        ClientSecret("secret-name", createdOn = FixedClock.now,  hashedSecret = "hashed-secret")
      val updatedApplication = await(
  ...
  "deleteClientSecret" should {
    "remove client secret with matching id" in {
      ...
      val clientSecretToRemove =
        ClientSecret("secret-name-1", createdOn = FixedClock.now, hashedSecret = "old-hashed-secret-1")
      val clientSecret2        =
        ClientSecret("secret-name-2", createdOn = FixedClock.now, hashedSecret = "old-hashed-secret-2")
      val clientSecret3        =
        ClientSecret("secret-name-3", createdOn = FixedClock.now, hashedSecret = "old-hashed-secret-3")
  ...
  "applyEvents" should {
    ...
    "handle AddClientSecret event correctly" in {
      ...
      val newClientSecret = ClientSecret("name", FixedClock.now, None, UUID.randomUUID().toString, "eulaVterces")
    ...
    "handle ResponsibleIndividualChanged event correctly" in {
      ...
      val importantSubmissionData = ImportantSubmissionData(None, oldRi, Set.empty,
        TermsAndConditionsLocation.InDesktopSoftware, PrivacyPolicyLocation.InDesktopSoftware, List(TermsOfUseAcceptance(oldRi, FixedClock.now, submissionId, submissionIndex)))
      ...
      val event = ResponsibleIndividualChanged(
        UpdateApplicationEvent.Id.random, applicationId, FixedClock.now, devHubUser,
    ...
    "handle ResponsibleIndividualVerificationStarted event correctly" in {
      ...
      val event = ResponsibleIndividualVerificationStarted(
        UpdateApplicationEvent.Id.random, applicationId, "app name", FixedClock.now, CollaboratorActor("admin@example.com"),
  ...
  "fetchProdAppStateHistories" should {
    def saveApp(state: State, timeOffset: Duration, isNewJourney: Boolean = true, environment: Environment = Environment.PRODUCTION) = {
      ...
      val app = anApplicationData(appId).copy(
        ...
        createdOn = FixedClock.now.plus(timeOffset),
    ...
    def saveHistory(appId: ApplicationId, maybeOldState: Option[State], newState: State, timeOffset: Duration) = {
      val stateHistory = StateHistory(appId, newState, OldActor("actor", ActorType.GATEKEEPER), maybeOldState, None, FixedClock.now.plus(timeOffset))
  ...
  def aNamedApplicationData(
      ...
      checkInformation: Option[CheckInformation] = None,
      clientSecrets: List[ClientSecret] = List(
        ClientSecret("", createdOn = FixedClock.now, hashedSecret = "hashed-secret")
      ),
      grantLength: Int = defaultGrantLength
    ): ApplicationData = {
