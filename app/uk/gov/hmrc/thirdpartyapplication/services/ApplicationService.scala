...
class ApplicationService @Inject() (
  ...
  def deleteApplication(
    ...
    (for {
      ...
      _   <- applicationRepository.delete(applicationId, LocalDateTime.now(clock))
  ...
  def createStateHistory(appData: ApplicationData)(implicit hc: HeaderCarrier) = {
    ...
    insertStateHistory(appData, appData.state.name, None, actor.id, actor.actorType, (a: ApplicationData) => applicationRepository.delete(a.id, LocalDateTime.now(clock)))
  }
