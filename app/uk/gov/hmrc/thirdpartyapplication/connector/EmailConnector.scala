...
class EmailConnector @Inject()(httpClient: HttpClient, config: EmailConnector.Config)(implicit val ec: ExecutionContext) extends ApplicationLogger {
  ...
  def sendVerifyResponsibleIndividualNotification(responsibleIndividualName: String,
    ...
    post(SendEmailRequest(Set(responsibleIndividualEmailAddress), verifyResponsibleIndividual,
      Map(
  ...
  def sendChangeOfApplicationName(requesterName: String, previousApplicationName: String, newApplicationName: String, recipients: Set[String])(implicit hc: HeaderCarrier): Future[HasSucceeded] = {
    post(SendEmailRequest(recipients, changeOfApplicationName,
      Map(
        "requesterName" -> requesterName, 
