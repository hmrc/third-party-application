...
class ResponsibleIndividualUpdateVerificationRemovalJob @Inject() (
  ...
  override def runJob(implicit ec: ExecutionContext): Future[RunningOfJobSuccessful] = {
    ...
    val result: Future[RunningOfJobSuccessful.type] = for {
      removalsDue <-
        repository.fetchByStateAgeAndTypes(ResponsibleIndividualVerificationState.INITIAL, removeIfCreatedBeforeNow, ResponsibleIndividualVerification.VerificationTypeUpdate, ResponsibleIndividualVerification.VerificationTypeTouUplift)
